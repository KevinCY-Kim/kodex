...
KevinCY-Kodex는 확장 가능한 AI 애플리케이션 구축을 위한 엔지니어링 표준 및 개발 가이드라인입니다.

본 문서는 아키텍처 설계부터 개발, 테스트, 배포에 이르는 소프트웨어 개발 생명주기(SDLC) 전반의 기준을 정의합니다. 단순한 규칙 나열이 아닌, 프로젝트의 일관성 유지와 협업 효율성을 높이기 위한 실무 중심의 프로토콜을 제공하는 것을 목적으로 합니다.

---

## 2. 핵심 철학

KevinCY-Kodex는 다음 세 가지 원칙을 준수합니다.

1.  **클린 아키텍처 (Clean Architecture)**: routers, services, repositories 등 계층 간의 명확한 분리를 통해 결합도를 낮추고 유지보수성을 확보합니다.
2.  **단일 진실 공급원 (Single Source of Truth, SSoT)**: 모든 규칙과 설정은 권위 있는 하나의 문서/파일에서 관리하여 정보의 중복과 불일치를 방지합니다.
3.  **전체 생명주기 포괄 (Full Lifecycle Coverage)**: 코드 작성뿐만 아니라 품질 보증(Testing)과 운영 자동화(CI/CD)까지 포함하는 포괄적인 표준을 지향합니다.

---

## 3. 문서구조 (Index)

각 영역별 세부 가이드는 아래의 문서들을 따릅니다.

### 기반 표준 (Foundational Standards)
-   **`README.md` (이 파일)**: 본 문서 (개요 및 사용법).
-   **`rules.md`**: 프로젝트 최상위 규칙 및 원칙 정의.
-   **`Architecture.md`**: 시스템 구성 요소 및 데이터 흐름도.
-   **`Folder_Standards.md`**: 디렉토리 구조 및 파일 위치 정의. (폴더 구조의 기준) **모든 폴더 관련 질문은 이 문서에서 답을 찾아야 합니다.**
-   **`Code_Style.md`**: 코딩 컨벤션, 포매팅, 네이밍 규칙.
...
신규 개발자는 `KODEX` 기반 프로젝트에 원활하게 적응하기 위해 다음 절차를 준수하여 프로젝트를 진행합니다.
...
KevinCY-Kodex의 목표는 불필요한 의사결정 비용을 줄이고, 견고하고 유지보수 가능한 AI 서비스를 지속적으로 제공하는 것입니다.
